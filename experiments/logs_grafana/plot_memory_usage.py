import pandas as pd
import matplotlib.pyplot as plt
import sys
import os
import re

if len(sys.argv) < 5:
    print(f"Usage: python3 {os.path.basename(sys.argv[0])} <plot_title> <start_time-end_time> <csv1> <csv2> [<csv3> ...]")
    sys.exit(1)

plot_title = sys.argv[1]
time_range = sys.argv[2]
csv_files = sys.argv[3:]

start_time, end_time = time_range.split("-")

fixed_colors = {
    "image-preprocess": "#0072B2",  # Blue
    "image-filter": "#E69F00",      # Orange
    "image-stats": "#009E73",       # Green
}

plt.figure(figsize=(10,5))

def parse_mem(val):
    # Example: "10.656 MiB", "1024 KiB", "123 B"
    match = re.match(r"([0-9.]+)\s*([a-zA-Z]+)", str(val))
    if not match:
        return float('nan')
    value, unit = float(match.group(1)), match.group(2)
    if unit == "MiB":
        return value
    elif unit == "KiB":
        return value / 1024
    elif unit == "B":
        return value / (1024 * 1024)
    else:
        # Fallback for possible "GiB" or unknown units
        if unit == "GiB":
            return value * 1024
        return float('nan')

for csv_file in csv_files:
    df = pd.read_csv(csv_file)
    mem_col = df.columns[1]
    func_name = mem_col.split('-')[0] + '-' + mem_col.split('-')[1]
    df["memory"] = df[mem_col].apply(parse_mem)
    df["Time"] = pd.to_datetime(df["Time"])
    mask = (df["Time"].dt.time >= pd.to_datetime(start_time).time()) & \
           (df["Time"].dt.time <= pd.to_datetime(end_time).time())
    df_filtered = df.loc[mask].copy()
    t0 = df_filtered["Time"].iloc[0]
    df_filtered["seconds"] = (df_filtered["Time"] - t0).dt.total_seconds()
    color = fixed_colors.get(func_name, "#333333")
    plt.plot(df_filtered["seconds"], df_filtered["memory"], label=func_name, color=color, linewidth=2)

plt.xlabel("Time [s]")
plt.ylabel("Memory Usage [MiB] (log scale)")
plt.yscale("log")

plt.title(plot_title)
plt.legend()
plt.grid(True, linestyle='--', alpha=0.6)
plt.tight_layout()

out_file = f"plot_ram_usage_{plot_title.replace(' ','_')}.pdf"
plt.savefig(out_file, dpi=300)
print(f"Plot saved as {out_file}")
